...
export default class HomePage extends React.Component {
	...
	loadPage(data) {
		...
		this.setState((state) => ({
			...
			total,
			totalFixed: data.expenses.filter((x) => x.priority === 'fixed').reduce((p, c) => p + c.debit_amount, 0),
			totalNecessary: data.expenses.filter((x) => x.priority === 'necessary').reduce((p, c) => p + c.debit_amount, 0),
			totalAvoidable: data.expenses.filter((x) => x.priority === 'avoidable').reduce((p, c) => p + c.debit_amount, 0),
			totalDoesntHurt: data.expenses.filter((x) => x.priority === 'one-off').reduce((p, c) => p + c.debit_amount, 0),
		}))
	}

	fetchMonthBalances() {
		fetch('/getMonthBalance' + '?startDate=' + new Date(this.state.date).toJSON())
			.then((data) => {
				return data.json()
			})
			.then((data) => {
		...
		if (expenses.length) {
			expenses.forEach((expense) => {
				if (
					...
					(['maintenance', 'netflix', 'max life ins', 'maid', 'gail'].includes(expense.expense_source) ||
						(expense.details.includes('EAW-512967XXXXXX5130') && expense.debit_amount === 10000) ||
			...
			expenses.forEach((expense) => {
				if (
					(expense.expense_source !== 'max life ins' && ['groceries', 'medical'].includes(expense.category)) ||
					['hair cut', 'petrol', 'thimmarayaswamy', 'office meal'].includes(expense.expense_source)
				) {
			...
			expenses.forEach((expense) => {
				if (['massage'].includes(expense.expense_source) || expense.expense_source?.includes('splurge')) {
					expense['priority'] = 'avoidable'
				}
			})

			expenses.forEach((expense) => {
				if (['car wash', 'zee5', 'donation', 'gifts', 'car service', 'loan', 'one-off'].includes(expense.expense_source) || expense.expense_source.includes('one-off')) {
	...
	render() {
		return (
			<div className='container p-3 m-auto'>
				<div className='row justify-content-start w-50'>
				...
				<div className='row p-2'>
					<SearchBar handleSearch={(c) => this.handleSearch(c)} handleClearSearch={() => this.handleClearSearch()}></SearchBar>
				</div>
...
function Table(props) {
	...
	if (props.dateString) {
		titleDateString = ' - ' + new Date(props.dateString).toLocaleString('en-US', { month: 'long' }) + ' ' + new Date(props.dateString).getFullYear()
	}
		...
		<div className='card border-0 table-responsive shadow'>
			<div className='card-header border-0 row white'>
				<div className='col-md-3 col-12 '>
					<h6 className='card-header-title h6 p-2 text-muted mb-0'>EXPENSES {titleDateString}</h6>
				</div>
				...
				<div className='col-md-3 col-12 row align-items-center'>
					...
					<div className='col-8'>
						<select id='selectPriority' className='form-select form-select-sm' onChange={(e) => props.handleSelectCategory(e.target.value, 'priority')}>
							<option defaultValue value=''>
								No filter
							</option>
		...
		<div className='card border-0 shadow'>
			<div className='card-header row white'>
				...
				<div className='col-2 text-end'>
					<i className='bi bi-chevron-down' type='button' data-bs-toggle='collapse' data-bs-target='#collapseExample'></i>
				</div>
			</div>
				...
				<div className='my-2 px-md-4'>
					...
					<div className='row justify-content-start my-2'>
						<div className='col-6 col-md-1'>
							<button className='btn btn-primary' disabled={!isDirty} onClick={(e) => handleSearch(searchCriterias)}>
								Search
							</button>
						</div>
						<div className='col-6 col-md-1'>
...
function SearchCriteria({ criteria, idx, handleAddCriteriaRow, handleDeleteCriteriaRow, handleChangeCriteria }) {
	return (
		<div className='row justify-content-start my-3'>
			<div className='col-12 col-md-auto mt-1'>
				<select className='form-select form-select-md' value={criteria.query.field} onChange={(e) => handleChangeCriteria(e.target.value, idx, 'field')}>
					<option defaultValue value=''>
						Select field
					</option>
			...
			<div className='col-12 col-md-auto mt-1'>
				<select className='form-select form-select-md col' value={criteria.query.operator} onChange={(e) => handleChangeCriteria(e.target.value, idx, 'operator')}>
					<option defaultValue value=''>
						Select operator
					</option>
			...
			<div className='col-12 col-md-auto mt-1'>
				<input className='form-control' type='text' placeholder='Enter search value' value={criteria.query.value} onChange={(e) => handleChangeCriteria(e.target.value, idx, 'value')}></input>
			</div>
			<div className='col-auto mt-1'>
				<button className={'btn border' + (criteria.operand === '$and' ? ' btn-primary' : '') } onClick={(e) => handleAddCriteriaRow(idx, '$and')}>
					And
				</button>
			</div>
			<div className='col-auto mt-1'>
				<button className={'btn border' + (criteria.operand === '$or' ? ' btn-primary' : '') } onClick={(e) => handleAddCriteriaRow(idx, '$or')}>
					Or
				</button>
			</div>
			{idx > 0 ? (
				<div className='col-auto mt-1'>
					<button className='btn border' onClick={(e) => handleDeleteCriteriaRow(idx)}>
